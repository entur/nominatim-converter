...
A GoSP's member stop names go in `address.other1`, `other2`, ... , which Photon routes to the document's context and indexes into `collector.all` only. They must not go back into `name.alt_name`: `SearchQueryBuilder.setupShortQuery` searches `collector.name`, and since a group's importance beats any member's, a generic word many members share ("bussterminal", "stasjon", "ferjekai") returned nothing but groups. The group stays reachable because every Norwegian GoSP is named after its locality and its members carry that name, so the group matches on its own name and the context terms add score. `Address::other` is `#[serde(flatten)]` - nested output would put the keys where Photon never looks.
